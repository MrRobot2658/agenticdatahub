...
A local Docker-Compose dev environment for a **multi-tenant real-time ID-Mapping + CDP (Customer Data Platform)** system, modeled on the production design in `docs/` (Kafka → Flink → MySQL → Doris). In dev, the heavy production components (Flink/StreamPark, Doris) are **simulated**: the `id-mapping` service stands in for Flink jobs, and MySQL stands in for Doris OLAP tables. **Scheduling, however, is real**: a single-container **Apache Airflow** service backs the "可视化编排 Pipelines" feature — `sql-engine`'s `scheduler_api.py` triggers a parametrized DAG (`airflow/dags/dataagent_pipeline.py`) over Airflow's REST API when a pipeline runs, passing the pipeline's node list in `dag_run.conf`; the DAG uses **dynamic task mapping** (`.expand`) to fan out one `run_node` task per canvas node at runtime (graceful local-sim fallback if Airflow is down). The frontend CDP console mimics Twilio Segment's app (IA & page design — see memory `mimic-segment.md`).
